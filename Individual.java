...
public class Individual {
    ...
    public Individual(Individual parent1, Individual parent2, int c_max, float m, int num_letters, Random rng) {
        ...
        int prefixlength=rng.nextInt(parent1.chromosome.size()-1)+1;
        int suffixlength=rng.nextInt(parent2.chromosome.size()-1)+1;
        for(int i=0; i<prefixlength;i++){
            prefix.add(parent1.chromosome.get(i));
        }
        for(int i=0; i<suffixlength;i++){
            suffix.add(0, parent2.chromosome.get(parent2.chromosome.size()-i-1));
        }
        chromosome=prefix;
